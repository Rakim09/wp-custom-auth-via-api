...
use CustomAuthViaApi\Common\Traits\Requester;
use CustomAuthViaApi\Common\Utils\Errors;
use CustomAuthViaApi\Config\Classes;
use CustomAuthViaApi\Config\I18n;
use CustomAuthViaApi\Config\Requirements;
...
final class Bootstrap extends Base {
	...
	public function initClasses() {
		$this->class_list = \apply_filters( 'custom_auth_via_api_initialized_classes', $this->class_list );
		foreach ( $this->class_list as $class ) {
			try {
				...
				\do_action( 'custom_auth_via_api_class_initialize_failed', $err, $class );
				Errors::wpDie(
					...
					__( 'Plugin initialize failed', 'custom-auth-via-api' ),
					__FILE__, $err
				);
			}
		}
	...
	public function getClasses( string $namespace ): array {
		...
		// Errors::writeLog(
		//    [
		//        'title'   => __( 'Custom Auth via API classes are not being loaded by Composer\'s Autoloader' ),
