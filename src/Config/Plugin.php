...
final class Plugin {
	...
	public function data(): array {
		$plugin_data = apply_filters( 'custom_auth_via_api_plugin_data', [
			'settings'               => get_option( 'custom-auth-via-api-settings' ),
			'plugin_path'            => untrailingslashit(
				plugin_dir_path( CUSTOM_AUTH_VIA_API_PLUGIN_FILE )  // phpcs:disable ImportDetection.Imports.RequireImports.Symbol -- this constant is global
			),
			'plugin_template_folder' => 'templates',
			'ext_template_folder'    => 'custom-auth-via-api-templates',
			/**
			 * Add extra data here
			 */
		] );
		return array_merge(
			apply_filters( 'custom_auth_via_api_plugin_meta_data',
				get_file_data( CUSTOM_AUTH_VIA_API_PLUGIN_FILE, // phpcs:disable ImportDetection.Imports.RequireImports.Symbol -- this constant is global
					[
						'name'         => 'Plugin Name',
						'uri'          => 'Plugin URI',
